...
class MyNotes {
  constructor() {
    if (document.querySelector("#my-notes")) {
      axios.defaults.headers.common["X-WP-Nonce"] = museumData.nonce
  ...
  events() {
    ...
    document.querySelector(".note-submit").addEventListener("click", () => this.createNote())
  }

  clickHandler(e) {
    if (e.target.classList.contains("note-delete") || e.target.classList.contains("fa-trash-can")) this.deleteNote(e)
    if (e.target.classList.contains("note-edit") || e.target.classList.contains("fa-pencil") || e.target.classList.contains("fa-times")) this.editNote(e)
    if (e.target.classList.contains("note-update") || e.target.classList.contains("fa-arrow-right")) this.updateNote(e)
  }
  ...
  makeNoteEditable(thisNote) {
    thisNote.querySelector(".note-edit").innerHTML = '<i class="fa fa-times" aria-hidden="true"></i> Cancel'
    thisNote.querySelector(".note-title-field").removeAttribute("readonly")
    thisNote.querySelector(".note-text-field").removeAttribute("readonly")
    thisNote.querySelector(".note-title-field").classList.add("note-active-field")
    thisNote.querySelector(".note-text-field").classList.add("note-active-field")
    thisNote.querySelector(".note-update").classList.add("note-update--visible")
    thisNote.setAttribute("data-state", "editable")
  }

  makeNoteReadOnly(thisNote) {
    thisNote.querySelector(".note-edit").innerHTML = '<i class="fa fa-pencil" aria-hidden="true"></i> Edit'
    thisNote.querySelector(".note-title-field").setAttribute("readonly", "true")
    thisNote.querySelector(".note-text-field").setAttribute("readonly", "true")
    thisNote.querySelector(".note-title-field").classList.remove("note-active-field")
    thisNote.querySelector(".note-text-field").classList.remove("note-active-field")
    thisNote.querySelector(".note-update").classList.remove("note-update--visible")
  ...
  async deleteNote(e) {
    ...
    try {
      const response = await axios.delete(museumData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"))
      ...
      if (response.data.userNoteCount < 5) {
        ...
        console.log('removed actuve class in note limit');
      }
    } catch (e) {
      console.log("Sorry")
    }
  ...
  async updateNote(e) {
    ...
    var ourUpdatedPost = {
      ...
      "content": thisNote.querySelector(".note-text-field").value
    }

    try {
      const response = await axios.post(museumData.root_url + "/wp-json/wp/v2/note/" + thisNote.getAttribute("data-id"), ourUpdatedPost)
      this.makeNoteReadOnly(thisNote)
    } catch (e) {
      console.log("Sorry")
    }
  ...
  async createNote() {
    var ourNewPost = {
      ...
      "content": document.querySelector(".new-note-text").value,
      "status": "publish"
    }

    try {
      const response = await axios.post(museumData.root_url + "/wp-json/wp/v2/note/", ourNewPost)
      ...
      if (response.data != "You have reached your note limit.") {
        ...
        document.querySelector("#my-notes").insertAdjacentHTML(
          ...
          ` <li data-id="${response.data.id}" class="fade-in-calc">
            ...
            <textarea readonly class="note-text-field">${response.data.content.raw}</textarea>
            <span class="note-update btn btn--blue btn--small"><i class="fa fa-arrow-right" aria-hidden="true"></i> Save</span>
          </li>`
        )
        ...
        console.log(response.data.content.raw)
      }
    } catch (e) {
      console.error(e)
    }

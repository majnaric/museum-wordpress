...
class Like {
  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios.post(museumData.root_url + "/wp-json/museum/v1/manageLike", { "exhibitionId": currentLikeBox.getAttribute("data-exhibition") })
  ...
  async deleteLike(currentLikeBox) {
    try {
      const response = await axios({
        url: museumData.root_url + "/wp-json/museum/v1/manageLike",
        method: 'delete',
